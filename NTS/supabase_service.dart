...
class SupabaseService {
  ...
  static Future<List<Map<String, dynamic>>> fetchSaves(String userId) async {
    final data = await client
        .from('saves')
  ...
  static Future<void> unsaveProduct(String userId, String productId) async {
    await client
        .from('saves')
        .delete()
        .eq('user_id', userId)
  ...
  static Future<List<Map<String, dynamic>>> fetchCart(String userId) async {
    final data = await client
        .from('cart_items')
        .select('*, products(*)')
        .eq('user_id', userId);
  ...
  static Future<void> sendMessage(
    ...
    await client.from('conversations').update(
        {'updated_at': DateTime.now().toIso8601String()}).eq(
        'id', conversationId);
  }
  ...
  static Future<void> topUpWallet(String userId, double amount) async {
    ...
    if (wallet == null) {
      await client.from('wallets').insert({'user_id': userId, 'balance': amount});
    } else {
    ...
    await client.from('topup_history').insert(
