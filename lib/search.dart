...
class _SearchPageState extends State<SearchPage> {
  ...
  Future<void> _search(String query) async {
    ...
    try {
      // Fetch all then filter locally (for simple case; use full-text search for large datasets)
    ...
    return Scaffold(
      ...
      body: _lastQuery.isEmpty
          ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text('Search for products', style: TextStyle(fontSize: 18, color: Colors.grey.shade600)),
              const SizedBox(height: 8),
              Text('Try "bike", "camera", "Dhaka"…', style: TextStyle(fontSize: 14, color: Colors.grey.shade500)),
            ]))
          : _searching
              ...
              : _results.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      const SizedBox(height: 16),
                      Text('No results for "$_lastQuery"', style: TextStyle(fontSize: 18, color: Colors.grey.shade600)),
                    ]))
                  : GridView.builder(
                      ...
                      itemBuilder: (_, i) => _SearchCard(product: _results[i]),
                    ),
    );
  }
}
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ClipRRect(
              ...
              child: Image.network(product.imageUrl, height: 120, width: double.infinity, fit: BoxFit.cover,
                  errorBuilder: (_, __, ___) => Container(height: 120, color: Colors.grey.shade300, child: const Icon(Icons.broken_image, size: 30))),
            ),
            const SizedBox(height: 8),
            ...
            Text('৳${product.price.toStringAsFixed(0)}', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
            const SizedBox(height: 2),
            if (product.listingType != null)
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                decoration: BoxDecoration(color: primaryColor.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
                child: Text(product.listingType == 'rent' ? 'Rent' : 'Buy',
