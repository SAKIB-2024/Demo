...
class SupabaseService {
  ...
  static Future<List<Map<String, dynamic>>> fetchProducts(
    ...
    // Filters must come before transforms — build filter chain first, then order.
  ...
  static Future<Map<String, dynamic>?> fetchProduct(String id) async {
    // Filters (.eq) must come before transforms (.order).
    final data = await client
        .from('products')
        .select(
        '*, profiles(full_name, avatar_url, nid_verified, is_banned), product_images(image_url, is_cover, sort_order)')
  ...
  /// Uploads one image for a product and inserts a row in product_images.
  /// Returns the public URL.
  static Future<String> uploadProductImage(
      String userId, String productId, Uint8List bytes,
      {bool isCover = false, int sortOrder = 0}) async {
    // Use microseconds + sortOrder to guarantee a unique filename
    ...
    // getPublicUrl is synchronous — always returns a stable, consistent URL
    ...
    // Keep products.image_url in sync with the cover so legacy fallback works
    if (isCover) {
      await client
          .from('products')
  ...
  /// Sets a specific product_images row as the cover (clears others first).
  ...
  /// Deletes a product image row and its storage file.
  ...
  /// [rentalDays] is used for rental items; pass 1 for buy items.
  static Future<void> addToCart(
      String userId, String productId, {int rentalDays = 1}) async {
    await client.from('cart_items').upsert({
      'user_id': userId,
      'product_id': productId,
      'quantity': 1,          // always 1; kept for schema compatibility
  ...
  static Future<List<Map<String, dynamic>>> fetchConversations(
    ...
    final data = await client
        .from('conversations')
        .select(
        '*, messages(text, created_at, sender_id), buyer:profiles!conversations_buyer_id_fkey(full_name, avatar_url), seller:profiles!conversations_seller_id_fkey(full_name, avatar_url), products(name)')
